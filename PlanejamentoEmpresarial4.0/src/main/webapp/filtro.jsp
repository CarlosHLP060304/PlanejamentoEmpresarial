...
<body>
	<main>
        ...
        <div class="d-flex justify-content-center gap-3 p-2">
            ...
            <button class="btn btn-primary open-btn" id="btn-filtrar" name="btn-filtrar">Filtrar <a class="text-danger text-decoration-none" href="index.jsp"> X</a></button>
        </div>
		<table class="table">
			<thead class="table-dark">
				<tr>
					<th>Descrição</th>
					<th>Data de Início da tarefa</th>
			...
			<tbody>
				...
				<tr>
					...
					<th><% Period periodo = tarefa.getDataInicio().until(tarefa.getDataFinalizacao());%>
						<%=periodo.getDays() %>
					</th>
					<% 
					String terminada = ""; 
		...
		<div class="pop-up p-2 none" id="pop-up">
			<form action="./tarefa" class="d-flex flex-column gap-3 p-3 bg-success rounded" method="post">
				...
				<fieldset class="">
					<label for="">Descrição da tarefa (Máx - 500 caracteres)</label>
			...
			<form action="./tarefa"
			 class="d-flex flex-column gap-3 p-3 bg-primary rounded" method="post">
				<div class="d-flex justify-content-start">
